...
self.addEventListener('message', event => {
  ...
  let private_key = event === null || event === void 0 ? void 0 : event.data.privateKey;
...
self.addEventListener('push', function (event) {
  ...
  event.waitUntil(registration.showNotification(data.title, {
    ...
    // 사용자 상호작용이 있을 때까지 알림 유지
    actions: [{
      action: 'explore',
      title: '서명하기',
      icon: '/icons/push/explore-icon.png'
    }, {
      action: 'site',
      title: '사이트 이동',
      icon: '/icons/push/explore-icon.png'
      ...
      action: 'close',
      title: '취소',
      icon: '/icons/push/close-icon.png'
    }]
  }));
});
